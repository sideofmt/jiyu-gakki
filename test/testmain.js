...
module.exports = function Testmain(){
    ...
    Test.write("\n\nserver stand up :"+dateinit.getFullYear()+":"+dateinit.getMonth()+":"+dateinit.getDate()+":"+dateinit.getHours()+":"+dateinit.getMinutes()+":"+dateinit.getSeconds()+":"+dateinit.getMilliseconds()+"\n");
    ...
    function noteOn(id,key){
      ...
      // times = [ID,key,"noteOn",time];
      
      var time = null;
      for(var i=0;i<times.length;i++){
        if(times[i][0]==id && times[i][1]==key && times[i][2]=="noteOn"){
          time = times[i][3];
          times.splice(i, 1);
          break;
        }
      ...
      broadcastSpeaker('noteOn',[id,key,time]);
    }
    ...
    function noteOff(id,key){
      ...
      var time = null;
      for(var i=0;i<times.length;i++){
        if(times[i][0]==id && times[i][1]==key && times[i][2]=="noteOff"){
          time = times[i][3];
          times.splice(i, 1);
          break;
        }
      ...
      broadcastSpeaker('noteOff',[id,key,time]);
    }
    ...
    io.sockets.on('connection', function (socket) {
      ...
      socket.on('noteOn',function(msg){
        var ID = socket.id;
        var time= msg[1];
        var key = msg[0]; // key,time,socketid
        if(player_sockets.indexOf(socket) != -1){
      ...
      socket.on('noteOff',function(msg){
        var ID = socket.id;
        var time= msg[1];
        var key = msg[0]; // key,testid,socketid
        ...
        if(player_sockets.indexOf(socket) != -1){
          ...
          times.push([ID,key,"noteOff",time]);
      ...
      */
      ...
      socket.on('note_back',function(msg){
        ...
        player_sockets.forEach(function (socket) {
          if(socket.id == playerid){
            socket.emit("note_back",[time,id,note]);
          }
        });
      ...
      socket.on('write_log',function(msg){
        ...
        console.log(index);
        var date = new Date();
        var datestr = date.getFullYear()+":"+date.getMonth()+":"+date.getDate()+":"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()+":"+date.getMilliseconds();
        
        var data = datestr+"     index:"+index+",  note:"+note+",  player id:"+pid+",  speaker id:"+sid+",  time:"+time+"\n";
        if(time!=null){
          Test.write(data);
        }
      });
